...
class PC:
    ...
    def makePattern(self, pattern, ctable):
        error("Auto pattern not supported.")
    ...
    def setSeqSize(self):
        ...
        self.autoPat       = seqBump(self.autoPat)
    ...
    def bar(self, ctable):
        ...
        """ Get pattern for this sequence. Either a Riff or Pattern.
            Patterns can be created on-the-fly if the auto key is set.
        """
        ...
        if self.riff:
            ...
            if not pattern:
                return
            if self.autoPat[sc]:
                self.makePattern(pattern, ctable)
                pattern = self.riff.pop(0)
