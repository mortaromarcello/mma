...
def doMidiTrackCresc(ln, dir, func):
    ...
    if v1 < 0 or v1 > 0x3fff:
        error("%s: Volume must be 0..16386." % (v1))
    ...
    ldur = int(count * gbl.BperQ * gbl.QperBar)
    ...
    if gbl.debug:
        print "MidiMasterVolume: Added %s changes" % (t)
...
def setMidiVolume(ln):
    ...
    if v < 0 or v > 16386:
        error("MidiVolume: Volume setting must be 0...16386.")
...
def trackWheel(name, ln):
    ...
    for o, v in opts:
        if o == 'OFFSET':
            startOffset = gbl.tickOffset + (int(stof(v) * gbl.BperQ))
            if startOffset < 0:
    ...
    if setOnly != None:
        if len(opts) >2 or len(opts) > 1 and startOffset == None:
    ...
    if startValue == None or endValue == None:
    ...
    if gbl.debug:
        print "MidiWheel %s: detuned %s to %s from %s to %s." % \
            (name, startValue, endValue, startOffset, endOffset),
        if not reset:
            print "No RESET."
        else:
            print
