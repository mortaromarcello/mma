...
class Chord(PC):
    ...
    def makePattern(self, pattern, ctable):
        """ Dynamically create a pattern based on the chord spacing. Use 
            the existing pattern for volumes.
        """
        ...
        for c in ctable:
            if c.name == 'z':
                continue
            ss = [p.offset for p in pattern]
            ctr=0
            vol = None
            for v1,v2 in zip(ss, ss[1:]+[999999]):
                if c.chStart >= v1 and c.chStart < v2:
                    vol=pattern[ctr].vol
                    break
                ctr+=1
            if not vol:
                ...
                vol = ' '.join([str(x) for x in vol])
            s += "%s %st %s;" % ((c.chStart/float(gbl.BperQ))+1, c.chEnd-c.chStart, vol)
        self.setRiff(s)
